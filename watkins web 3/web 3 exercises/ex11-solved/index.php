...
// if (isset($_GET['form-submit'])) {
	...
	// $keyword = "washington";
	$keyword = $_GET["term"];
	// lets create a "URL-safe"version of the keyword that replaces spaces with the + symbol. Yahoo will like that.
	$keyword_no_spaces = str_replace(" ", "+", $keyword);
	$feed_url = ("http://news.search.yahoo.com/news/rss?p=$keyword_no_spaces");
	$rawfeed = file_get_contents("$feed_url");
	...
	// User feedback:
	echo ("You searched for <em>$keyword</em>:");
	echo ("<ul>");
	// Lets display 10 items
	for ($i = 0; $i <= 9; $i++) {
		$thisheadline = $item[$i]->title;
