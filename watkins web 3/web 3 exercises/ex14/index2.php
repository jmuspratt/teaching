...
<nav><p><a href="index.php">Part 1</a> | <a href="index2.php">Part 2</a></p></nav>
...
// Upload the exercise and run it. Read through the code below and make sure you understand each step.
	...
	// ----- Get form input
		...
		if ($search) {
			echo ("<p>Showing only contributions from <strong>$search</strong>.</p>"); 
	...
	function show_contributions($table_name) {
		...
		if ($search) {
			...
			$results = mysql_query("SELECT * FROM $table_name WHERE contbr_city = '$search' AND contb_receipt_amt > 0 ORDER BY contb_receipt_amt ASC");
			} // end if condition
